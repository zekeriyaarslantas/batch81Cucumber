...
public class AmazonStepDefinition {
    ...
    @When("url'nin {string} icerdiğini test edelim")
    public void urlNinIcerdiğiniTestEdelim(String istenenUrl) {
        String actualUrl=Driver.getDriver().getCurrentUrl();
        Assert.assertTrue(actualUrl.contains(istenenUrl));
    }

    @Then("kullanici {int} saniye bekler")
    public void kullaniciSaniyeBekler(int istenenSüre) {
        try {
            Thread.sleep(istenenSüre*1000);
